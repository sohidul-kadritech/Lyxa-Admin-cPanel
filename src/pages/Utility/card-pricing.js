      ...
      <Card className="pricing-box">
        <CardBody className="p-4">
          ...
          <div className="pricing-features mt-5 pt-2">
            {props.pricing.features.map((feature) => (
              <p key={`_feature_${Math.random()}`}>
                <i className={`${feature.icon} text-primary me-2`} /> {feature.title}
              </p>
            ))}
          </div>
          ...
          <div className="d-grid mt-5">
            <Link to="#" className="btn btn-primary btn-block waves-effect waves-light">
              Sign up Now
            </Link>
          </div>
        </CardBody>
